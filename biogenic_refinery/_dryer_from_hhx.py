...
data_path = os.path.abspath(os.path.dirname('_dryer_from_hhx.csv'))
...
# in _run: define influent and effluent streams and treatment processes 
    def _run(self):
        ...
        waste_out.imass['H2O'] = (0.35/0.65) * TS_in # fraction
        ...
        # emissions
        drying_CO2_to_air = self.drying_CO2_emissions * waste_in.COD * waste_in.F_vol / 1000 # kg CO2 /hr

        # add conversion factor for COD to TC?
        drying_CH4_to_air = self.drying_CH4_emissions * waste_in.COD * waste_in.F_vol / 1000 # kg CH4 /hr
        drying_NH3_to_air = self.drying_NH3_emissions * waste_in.TN * waste_in.F_vol / 1000 # kg NH3 /d
        drying_N2O_to_air = drying_NH3_to_air * self.NH3_to_N2O # kg N2O /d
        ...
        jacket_heat_loss_conv = self.heat_transfer_coeff * (self.water_air_hx_temp1 - self.ambient_temp) * self.water_air_hx_area1 / 1000 / 0.2778 # MJ/hr
        jacket_heat_loss_radiation = self.radiative_emissivity * 5.67e-8 * ((self.water_air_hx_temp1 + 273)**4 - (self.ambient_temp + 273)**4) / 1000 / 0.2778 # MJ/hr
        jacket_heat_loss_sum = jacket_heat_loss_conv + jacket_heat_loss_radiation # MJ/hr
