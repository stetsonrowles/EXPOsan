...
#data_path = '/Users/lewisrowles/Dropbox/My Mac (Lewiss-MBP.lan1)/Documents/UIUC/Gates/QSDsan/QSD_adding_biogenic_refinery/_hydronic_heat_exchanger.csv' 
...
class HydronicHeatExchanger(SanUnit):
    ...
    def _run(self):
        ...
        hot_gas_out = self.outs
        hot_gas_in.phase = hot_gas_out.phase = 'g'
        
        # set temperature
        hot_gas_out.T = self.hhx_temp
        # calculate the heat that was delivered to the HHX
        heat_output_water = self.water_flowrate *  ((273.15 + self.water_out_temp) - (273.15 + self.water_in_temp)) * self.water_density_kg_m_3 * self.water_heat_capacity_k_j_kg_k * (60/1000)  # MJ/hr
        # calculate losses through water pipe
        heat_loss_water_pipe = ((273.15 + self.water_out_temp) - (273.15 + self.ambient_temp)) / (self.water_r_pipe_k_k_w + self.water_r_insulation_k_k_w) * 3.6 # MJ/hr
