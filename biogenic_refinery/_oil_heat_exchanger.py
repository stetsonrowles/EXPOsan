...
#data_path = '/Users/lewisrowles/Dropbox/My Mac (Lewiss-MBP.lan1)/Documents/UIUC/Gates/QSDsan/QSD_adding_biogenic_refinery/_oil_heat_exchanger.csv' 
...
class OilHeatExchanger(SanUnit):
    ...
    def _run(self):
        heat_in = self.ins[0]
        heat_out = self.outs
        heat_in.phase = heat_out.phase = 'g'
        
        # set temperature
        heat_out.T = self.ohx_temp

        # calculate the power that was delivered to the ORC
        power_delivery_orc = self.oil_flowrate * ((273.15 + self.oil_temp_out) - (273.15 + self.oil_temp_in)) * self.oil_density * self.oil_specific_heat * (60/1000)  # MJ/hr
        # calculate losses through pipe
        pipe_heat_loss = ((273.15 + self.oil_temp_out) - (273.15 + self.amb_temp)) / (self.oil_r_pipe_k_k_w + self.oil_r_insulation_k_k_w) * 3.6  # MJ/hr
